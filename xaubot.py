...
TIMEFRAMES = {
    "Cố (W)":    {"twelve": "1week", "label": "W",   "weight": 5},
    "Ông (3D)":  {"twelve": "3day",  "label": "3D",  "weight": 4},
    "Cha (D1)":  {"twelve": "1day",  "label": "D1",  "weight": 3},
    "Con (H12)": {"twelve": "12h",   "label": "H12", "weight": 2},
    "Cháu (H4)": {"twelve": "4h",    "label": "H4",  "weight": 1},
}
...
def fetch_candles(interval, outputsize=100):
    ...
    try:
        ...
        if data.get("status") == "error":
            print(f"⚠️ Twelve Data lỗi [{interval}]: {data.get('message')}")
            return None
        ...
        print(f"⚠️ Lỗi fetch [{interval}]: {e}")
        return None
...
def detect_form(df):
    if df is None or len(df) < 50:
        return {"direction": None, "diem": 0, "pct": 0, "rsi": None,
                "ma9": None, "ma45": None, "price": None,
    ...
    if pd.isna(ma9) or pd.isna(ma45) or pd.isna(rsi):
        return {"direction": None, "diem": 0, "pct": 0, "rsi": rsi,
                "ma9": ma9, "ma45": ma45, "price": price,
                "gap_pct": 0, "gap_status": "N/A", "trigger": "Không đủ dữ liệu"}
    ...
    gap_abs_now  = abs((ma9 - ma45) / ma45 * 100)
    gap_abs_prev = abs((prev["ma9"] - prev["ma45"]) / prev["ma45"] * 100)
...
def build_trade_plan(results, direction, score, cascade_info):
    ...
    if score >= MIN_SCORE and n >= 3:
        ...
        atr = max(abs(ma9_h4 - ma45_h4) * 2, price * 0.008)
        if direction == "SELL":
...
def format_message(results, score, direction, cascade_buy, cascade_sell):
    ...
    lines += build_trade_plan(results, direction, score, cascade_info)
    lines += ["", f"{'─' * 28}", "⚡ _XAU Bot | Twelve Data_"]
...
def run(force=False):
    if not TWELVE_KEY:
        print("⚠️  Chưa set TWELVE_API_KEY!")
        return
    ...
    if score >= MIN_SCORE or force:
        msg = format_message(results, score, direction, cascade_buy, cascade_sell)
        send_telegram(msg)
    else:
